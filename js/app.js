const TYPES = {
    ...
    function installPlatformStyles() {
      ...
      style.textContent = `
        ...
        .hero-copy .home-hero-title { display: block; width: 100%; max-width: none; color: transparent; font-family: "Trebuchet MS", "Segoe UI", Arial, sans-serif; font-size: clamp(2.6rem, 5.6vw, 6.7rem); font-style: normal; font-weight: 900; letter-spacing: -.075em; line-height: .95; white-space: nowrap; text-wrap: nowrap; perspective: 900px; text-shadow: 0 1px 0 rgba(255,255,255,.78), 0 9px 18px rgba(11,52,72,.14); }
        .hero-letter { position: relative; display: inline-block; color: rgba(15,65,84,.9); background: linear-gradient(180deg, rgba(255,255,255,.92) 0%, rgba(180,235,242,.88) 30%, rgba(54,145,169,.92) 65%, rgba(14,67,88,.94) 100%); -webkit-background-clip: text; background-clip: text; -webkit-text-stroke: 1px rgba(255,255,255,.6); filter: drop-shadow(0 3px 4px rgba(7,66,91,.15)); transform: translate3d(0, 0, 0) rotateZ(0deg) scale(1); transform-origin: 50% 72%; transition: transform .28s cubic-bezier(.2,.85,.22,1), filter .28s ease; will-change: transform; }
        .hero-letter::after { display: none; }
        .hero-letter:hover, .hero-letter:focus-visible { filter: drop-shadow(0 8px 12px rgba(0,112,145,.24)); transform: translate3d(0, -8px, 12px) rotateZ(var(--letter-tilt, -2deg)) scale(1.045); }
