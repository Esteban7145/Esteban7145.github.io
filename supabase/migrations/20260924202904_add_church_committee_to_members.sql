alter table public.church_members
  add column if not exists church_committee text;
...
alter table public.member_change_requests
  drop constraint if exists member_change_requests_document_type_check,
  drop constraint if exists member_change_requests_document_number_check;

alter table public.member_change_requests
  add constraint member_change_requests_document_pair check (
...
alter table public.church_members
  drop constraint if exists church_members_baptism_date_check,
  add constraint church_members_baptism_date_check check (is_baptized is true or baptism_date is null);

alter table public.member_change_requests
  drop constraint if exists member_change_baptism_date_check,
  add constraint member_change_baptism_date_check check (is_baptized is true or baptism_date is null);
